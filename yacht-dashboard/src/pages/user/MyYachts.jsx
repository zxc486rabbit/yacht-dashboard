...
const MyYachts = () => {
  ...
  const visaTotal =
    Number(yachtData.visa.onboardTaiwan || 0) +
    Number(yachtData.visa.onboardForeign || 0);
  ...
  const validateField = (section, field, value) => {
    ...
    // 數字
    if (["grossTonnage", "loa", "beam", "depth", "draft", "crewHealthIssueCount", "onboardTaiwan", "onboardForeign"].includes(field)) {
      if (value && isNaN(Number(value))) msg = "必須為數字";
    }

    // 日期
    if (["buildDate", "arrivalDate", "departureDate"].includes(field)) {
    ...
    <div className="yacht-page">
        ...
        <div className="grid-2">
          <div>
            <FormRow
              label="預定進港日期"
              type="date"
              value={yachtData.schedule.arrivalDate}
            ...
            <FormRow
              label="預定離臺日期"
              type="date"
              value={yachtData.schedule.departureDate}
            ...
            <FormRow
              label="進港目的"
              value={yachtData.schedule.purpose}
              onChange={(v) => handleChange("schedule", "purpose", v)}
              disabled={!editMode}
            />
            <FormRow
              label="前一港代碼"
              value={yachtData.schedule.previousPortCode}
              onChange={(v) => handleChange("schedule", "previousPortCode", v)}
              disabled={!editMode}
            />
            <FormRow
              label="進出港口"
              value={yachtData.schedule.portCode}
          ...
          <div>
            <FormRow
              label="預定進港時間 (HHMM)"
              value={yachtData.schedule.arrivalTime}
              onChange={(v) => handleChange("schedule", "arrivalTime", v)}
              disabled={!editMode}
            />
            <FormRow
              label="次一港代碼"
              value={yachtData.schedule.nextPortCode}
              onChange={(v) => handleChange("schedule", "nextPortCode", v)}
              disabled={!editMode}
            />
            <FormRow
              label="靠泊馬頭"
              value={yachtData.schedule.berth}
      ...
      <section className="yacht-section">
        ...
        <div className="grid-2">
          ...
          <FormRow
            label="船員健康異常人數"
            value={yachtData.other.crewHealthIssueCount}
      ...
      <section className="yacht-section">
        ...
        <div className="grid-3">
          <FormRow
            label="在船人數 (台灣)"
            value={yachtData.visa.onboardTaiwan}
          ...
          <FormRow
            label="在船人數 (外國)"
            value={yachtData.visa.onboardForeign}
